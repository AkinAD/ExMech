...
/**
 * This class provides abstract and concrete methods for simulating a braille
 * cell.
 * <p>
 ...
 * Player, some implemented and others remain abstract for future
 * implementation.
 * 
...
public abstract class Player {
	 ...
	 * for all Player subclasses, such as checking the validity of
	 * brailleCellNumber and buttonNumber (non-negative), as well as creating
	 * the specified number of BrailleCell objects and adding it to the
	 * <code>brailleList</code> List.
	 * 
	...
	public Player(int brailleCellNumber, int buttonNumber) {
		...
		// Formatting the Logger for the player class, which all its child
		// classes uses.
		// Change the formatting as needed.
		// Currently, it's set to ConsoleHandler instead of FileHandler. It will
		// write the log
		// to the console.
		// Eventually, we'll need to have it save to a file. Simply change
		// ConsoleHandler to FileHandler,
		// and set the output to the appropriate directory.

		// To find out what's being logged, search and find any "logger.log"
		// calls.
	 ...
	 * its BrailleCells, whether the display is audio, visual, or hardware, or
	 * other implementations that extend this class.
	 */
	public abstract void refresh();

	/**
	 * Adds a "Skip Button" for the ScenarioParser's class to continue
	 * processing text after the ScenarioParser's <code>userInput</code> is set
	 * to true. Changes <code>userInput</code> to false to allow the
	...
	/**
	 * Adds a "Repeat Button" for the ScenarioParser's class to repeat the text.
	 * 
	 * @param index
	 *            the index of the button to add the listener to
	 * @param sp
